...
function renderNews() {
  ...
  let html = newsDataRender.map((newsData) => {
    let result = tpl.replace("{{title}}", newsData.title)
      .replace("{{content}}", newsData.content);
    return result;
  }).join("");
...
async function run() {
  ...
  skipInput.onkeydown = function (e) {
    if (e.keyCode == 13) {
      p = parseInt(skipInput.value);
      if (p > 0 && p <= asAll.length) {
        changePageClass(p);
        renderNews();
      }
    }
